...
- id      : 학생 고유 번호
- name    : 학생 이름
- score   : 각 과목별 성적을 저장하는 객체 { Math: 85, English: 90, Science: 78 }
...
class Student {
  ...
  getAverageScore() {
    // Object.values(객체);
    // >> 전달된 객체가 가지는 속성의 값들로만 이루어지 배열을 가짐
    //? cf) 객체(key): 값(value)의 한 쌍으로 이루어짐
...
class GradeManagement {
  ...
  addStudent(name, score) {
    const newStuentnt = new Student(this.nextId, name, score);
    this.students.push(newStuentnt);
    console.log(`학생 추가: [${newStuentnt.id}] ${newStuentnt.name}`);
    this.nextId++;
  }
...
// 학생 추가 예제
gradeSystem.addStudent("이승아", { Math: 90, English: 85, science: 78});
gradeSystem.addStudent("이도경", { Math: 80, English: 95, science: 88});
gradeSystem.addStudent("조승범", { Math: 85, English: 100, science: 65});
gradeSystem.addStudent("김명진", { Math: 70, English: 80, science: 87});
