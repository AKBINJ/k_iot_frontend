...
class Book {
  ...
  constructor(id, title, author) {
    // 생성자 함수에서 this 키워드로 속성 정의된는 값(좌항)은 필드로 자동 정의
    this.id = id;
    this.title = title;
    this.author = author;
...
class Library {
  ...
  addBook(title, author) {
    ...
    console.log(`${title} 책이 도서관이 추가되었습니다. (저자: ${author})`);
    this.nextBookId++;
  }
  ...
  rentBook(id) {
    ...
    if (book) {
      // 해당 책이 도서관에 등록되어 있는 경우 - 각 객체의 내부 메서드 호출
      book.rentBook();
    } else {
  ...
  returnBook(id) {
    ...
    if (book) {
      // 해당 책이 도서관에 등록된 경우 (책 반납 가능가능 여부는 미정)
      book.returnBook();
    } else {
      console.log('해당하는 책을 찾을 수 없습니다.');
    }
  ...
  updateBook(id, newTitle, newAuthor) {
    // +) find 메서드는 배열에서 제공된 콜랩함수를 만족하는 첫 번째 요소를 반환
  ...
  removeBook(id) {
    ...
    if (idx !== -1) {
      const removeBook = this.books.splice(idx, 1)[0];
  ...
  // [통계] 대여 가능 도서 수 집계
  countAvailableBooks() {
    // isAvailabledl true인 데이터만 추출하여 해당 배열의 길이를 측정
...
//! 3. 프로젝트 실행
// == 부산 도서관 ==
const busanLibray = new Library();

busanLibray.addBook('자바스크립트 공부', '이승아');
busanLibray.addBook('자바 공부', '이도경');
busanLibray.addBook('SQLD 공부', '이지희');
busanLibray.addBook('타입스크립트 공부', '이지훈');
busanLibray.addBook('리엑트 공부', '이승아');

busanLibray.displayBook();

busanLibray.rentBook(1);
busanLibray.displayBook();
busanLibray.returnBook(1);
busanLibray.displayBook();

busanLibray.updateBook(2, 'Java 공부 재미', null);
busanLibray.updateBook(2, 'Java study is fun', 'Lee do kyeong');

busanLibray.removeBook(3);
busanLibray.displayBook();

// == 양산 도서관 ==
const yangsanLibray = new Library();

yangsanLibray.addBook('자바스크립트 공부', '이승아');
yangsanLibray.addBook('자바 공부', '이도경');
yangsanLibray.addBook('SQLD 공부', '이지희');
yangsanLibray.addBook('타입스크립트 공부', '이지훈');
yangsanLibray.addBook('리엑트 공부', '이승아');

yangsanLibray.displayBook();

yangsanLibray.filterBooksByAuthor('이승아');

yangsanLibray.filterBooksByTitle('스크립트');

yangsanLibray.rentBook(1);
yangsanLibray.rentBook(3);
yangsanLibray.filterBooksByAvailable(true);
yangsanLibray.filterBooksByAvailable(false);

yangsanLibray.countAvailableBooks();
