...
//? cf) 타입 별칭(type alias)
// : 데이터 타입 명시
// - 윈시 타입, 유니온, 인터섹션 등의 모든 타입을 별칭으로 지정 가능
type UserType = {
  name: string;
  age: number;
}
...
// : interface 키워드 사용
// - 인터페이스명은 대문자로 시작
// - 객체의 문법 체계 모두 사용 가능 (옵셔널 ?, 읽기전용 readdonly)
interface IUser {
  // 인터페이스명: I+타입명
  // 타입별칭: 타입명Type
  name?: string;
...
class Student implements IUser {
  ...
  //    > "컴파일러 내가 책임짐 나중에 반드시 초기화 됨"
  //    > Definiton Assignment Assertion (확실한 할당 주장)
  name!: string;
  age: number;
...
interface Square extends Shape {
  sideLength: number;
  // color: string; - 해당 속성이 생락된 것처럼 작동
}
