...
function UpdateUser(id: number, changes: Partial<User>) {
  ...
  // 1) name만 있는 경우: { 기본의name, 기존의age, 변결될name } >> changes의 name값으로 수정
  // 2) age만 있는 경우: { 기존의name, 기존의age, 변경될age } >> changes의 age 값으로 수정
...
const genericA: EmployeeForGeneral = {
  id: 1,
  name: '박진영',
  age: 30,
  position: '교육부'
