...
const API_BASE =
  import.meta.env.VITE_APP_API_BASE || "http://localhost:8080/api/v1";
...
export const api: AxiosInstance = axios.create({
  baseURL: API_BASE,
  timeout: 10000, //10초 타임아웃 - 요청이10초이상걸릴 경우 초기화
  headers: {
...
//? Axios 특징
// 1) Promise 기반: .소두() / swait 사용 가능
// 2) 자종 JSON 처리 : 객체를 JSON으로 변환하거나 파싱 자동
// 3) 응답 객체 구조화: response.data만 확인
// 4) 인터셉터 지원: 요청 / 응답 가로채기 가능 (토큰/ 에러 처리 등)
// 5) Axios 인스턴스: 콩통 설정을 모아 재사용 가능
...
//! 응답 인터셉터: 401 처리 (Unauthrized: 자격 증명 부족), 에퍼 포맷 통일
// api.interceptors.response.use(응답 설정이행, 응답설정에러)
