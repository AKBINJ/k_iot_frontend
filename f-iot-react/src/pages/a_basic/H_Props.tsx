...
//! Wrapper 컴포넌트
// : 다른 컴포넌트를 감싸는 컴포넌트
// - props로 데이터로 다른 컴포넌트(ReactNode)를 전달 받음
// - 자식 컴포넌트를 안전하게 받기 위한 타입

type ChildrenType = {
  // ReactNodeL: React 내의 HTML 요소들 + 사용자 정의 컴포넌트들의 타입
...
function H_Props() {
  ...
  return (
    <div>
      {/* 
        == 콘솔창 / 컴포넌트 모두 두번 실행 ==
        : React18 이후의 StrictMode가 개발 모두에서 부작용 탐지를 위해 두 번 렌더링
        > 실제 배포 환경에서 는 한 번만 렌더링
      */}
