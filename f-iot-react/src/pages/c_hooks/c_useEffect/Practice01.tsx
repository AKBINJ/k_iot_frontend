...
function Practice01() {
  ...
  const updateReservationStatus = async (id: number, newStatus: ReservationStatus) => {
    try {
      ...
      // UI 즉시 반영
      setReservations(prev => prev.map(reservations => 
        reservations.id === id ? { ...reservations, status: newStatus } : reservations
      ));
    } catch (e) {
    ...
    <div style={{ padding: '20px', backgroundColor: '#f5f5f5', borderRadius: '12px'}}>
      ...
      <ul>
        {reservations.map(reservations => (
          <li key={reservations.id} style={{
            background: 'white',
            margin: '10px 0',
            padding: '10px',
            borderRadius: '8px'
          }}>
            <h4>{reservations.customer_name} ({reservations.truck_name})</h4>
            <p>시간대: {reservations.time_slot}</p>
            <p>상태: {" "} <b style={{
              color: reservations.status === 'CONFIRMED'? 'green' :
                      reservations.status === 'PENDING' ? "orange" : 'red'
            }}>{reservations.status}</b></p>

            {/* 상태 변경 버튼 영역 */}
            <div style={{ marginTop: '8px' }}>
              {/* CONFIRMED가 아닐 때만 승인 버튼 */}
              {reservations.status !== 'CONFIRMED' && (
                <button onClick={() => updateReservationStatus(reservations.id, "CONFIRMED")}>
                  승인
                </button>
              )}
              ...
              {reservations.status !== 'CANCELLED' && (
                <button onClick={() => updateReservationStatus(reservations.id, "CANCELLED")}>
                  취소
                </button>
              )}
            </div>
