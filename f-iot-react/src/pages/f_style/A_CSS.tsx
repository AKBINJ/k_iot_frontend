...
// - 장점) 빠르고 단순함
// - 단점) 클래스 충돌 윟머, 컴포넌트 단위 관리 어려움
//@ "Global CSS"로써 사용 시
// - css파일을 전역으로 import 하여, 모든 컴포넌트에 스타일 적용 

//? 2. 일반 CSS 사용 방법
// : 별도의 설정 필요 없이 .css 파일 import
// cf) src/styles 폴더를 두고 글로벌 스타일을 체계적으로 관리하는 것이 실무 표준

//? 3. 일반 CSS를 Global CSS로 사용한느 방법
// - reset.css      : 초기화 (UI 프레임워크와 상관없이 필수)
// - variables.css  : css 변수 정의 (색상, spacing, typography 등 디자인 시스템을 변수로 관리)
...
// 4) 페이지 일관성
// 5) 유지보수: 컴포넌트 단위가 많아지면 CSS Moudule, styled-components로 전환


function A_CSS() {
  return (
    <div>
      {/* 바로 적용 가능, 빠르고 직관적 / 클래스명 출동 가능성 */}
